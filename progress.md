...
```
N1 (Select Model Elements) → N3 → N9 → N10
                           → N4 → N5 → N8
                                └→ N6 → N9
                           → N7 → N9
N2 → N8
...
**Lưu ý:** Tất cả node N3-N9 đều hỗ trợ list input. Có thể nối trực tiếp N1 → N3 → N4 → N5 → N6 → N7 → N8 → N9 → N10 để chạy batch tự động.
...
- **Trạng thái:** ⚠️ Cần debug thêm — user báo "vẫn đúng" nhưng N10 báo failed
...
1. **Debug N10:** User báo annotation đúng nhưng N10 báo failed. Cần kiểm tra:
   - Width parameter có đúng là Type parameter không?
   - Length tính từ LocationCurve có khớp với width + 40mm không?
   - Có thể do tolerance quá chặt (1mm) hoặc parameter name khác?

## Cách sử dụng

### Single element (test)
```
N1 → List.GetItemAtIndex(0) → N3 → N4 → N5 → N6 → N7 → N8 → N9 → N10
N2 → N8
```
